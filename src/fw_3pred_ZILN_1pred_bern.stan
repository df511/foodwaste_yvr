...
data {
  ...
  vector[N] X3;    // Predictor: Proportion of renters (or other predictor)
...
parameters {
  ...
  real beta3;               // Coefficient for X1 in lognormal model
...
transformed parameters {
  vector[N] log_mean = mu + beta1 * X1 + beta2 * X2 + beta3 * X3;  // Lognormal predictor
  vector<lower=0,upper=1>[N] pi = inv_logit(gamma + delta1 * P1);  // Zero-inflation probability
}

// Model block
model {
  // Priors
  mu ~ normal(-4.5, 3);            // lower and wider prior
  beta1 ~ normal(0, 1);          // Slightly wider prior
  beta2 ~ normal(0, 1);          // Slightly wider prior
  beta3 ~ normal(0, 1);          // Slightly wider prior
  sigma ~ normal(0, 1);         // Weakly informative prior

  gamma ~ normal(0.86, 0.15);        // Prior for zero-inflation intercept
