...
data {
  ...
  real X3[N];  // Predictor: Proportion of renters (or other continuous variable)
...
parameters {
  ...
  real beta2;  // Coefficient for the effect of X2 (proportion of renters or other predictor)
  real beta3;  // Coefficient for the effect of X3 (proportion of renters or other predictor)
...
transformed parameters {
  ...
  for (n in 1:N) {
    log_mean[n] = mu + beta1 * X1[n] + beta2 * X2[n]+ beta3 * X3[n];  // Linear predictor on log scale
  }
}
...
model {
  ...
  beta2 ~ normal(0, 1);     // Prior for the effect of X2 (predictor)
  beta3 ~ normal(0, 1);     // Prior for the effect of X3 (predictor)
  sigma ~ normal(0, 1);    // Weakly informative prior for standard deviation
  pi ~ beta(2, 8);         // Prior for zero-inflation probability (expect 85% zeros)
