...
fw_scores <- dat_sf_joined[, .(fw_score_weighted = sum(weighted_value, na.rm = TRUE),
                               fw_score_max = max(weighted_value, na.rm = TRUE)), 
                           by = .(grid_id, site, date, pickup_type)]  # Replace grid_id with actual column name
...
# Step 1: All expected survey visit combos per site
expected_visits <- surv_dat %>%
  distinct(site, date, pickup_type)


# Step 2: Grid cells that only contain zeros
NA_only_grids <- dat_grid %>%
  group_by(grid_id) %>%
  filter(all(is.na(fw_score_weighted) & is.na(fw_score_max))) %>%
  slice(1) %>%  # Just keep one row per group
  ungroup() %>%
  select(grid_id, name)

# Step 3: Cross expected visits with zero-only grids by site
missing_expected_combos <- NA_only_grids %>%
  left_join(expected_visits, by = c("name" = "site"))  # ensures all expected visits per site are included

### test to make sure no visits greater than expected
# test <- missing_expected_combos %>%
#   distinct(grid_id, date, pickup_type) %>%
#   count(grid_id, name = "n_types") 


# Step 4: Identify which rows are missing in dat_grid
existing_keys <- dat_grid %>%
  st_drop_geometry() %>%
  select(grid_id, site, date, pickup_type)

# Rename 'name' to 'site' in missing_expected_combos to match existing_keys
missing_rows <- missing_expected_combos %>%
  rename(site = name) %>%
  anti_join(existing_keys, by = c("grid_id", "site", "date", "pickup_type")) %>%
  mutate(
    fw_score_weighted = 0,
    fw_score_max = 0
  )
...
# Step 5: Recombine and attach geometry
dat_grid <- bind_rows(
  dat_grid %>% st_drop_geometry(),
  missing_rows %>% st_drop_geometry()  # remove geometry here
) %>%
  left_join(
    dat_grid %>% select(grid_id, geometry) %>% distinct(),
    by = "grid_id"
  ) %>%
  st_as_sf()
...
dat_grid <- dat_grid %>%
  # Step 1: Drop rows where the 'site' column is NA
  filter(!is.na(site)) %>%
  
  # Step 2: Drop the 'name' column
  select(-name) %>%
  
  # Step 3: Rename 'site' column to 'name'
  rename(name = site)


#### Save and name file!!
save(dat_grid, file = here("data","dat_grid_750_pickuptypes.Rdata"))

#######
} else {
## object called "dat_grid"
load(file = here("data", "dat_grid_750_pickuptypes.Rdata"))
}
...
save(dat_grid, file = here("data","dat_fw_demo_750_pickuptypes.Rdata"))
} else {
  ## object called "dat_grid"
  load(file = here("data", "dat_fw_demo_750_pickuptypes.Rdata"))
}
...
### save file
save(dat_grid, file = here("data","dat_fw_demo_lc_750_pickuptypes.Rdata"))

} else {
  ## object called "dat_grid"
  load(file = here("data", "dat_fw_demo_lc_750_pickuptypes.Rdata"))
}
...
dat_grid_final <- dat_grid_final[,-c(142:146)]
...
save(dat_grid_final, file = here("data","dat_fw_demo_lc_retail_750_pickuptype.Rdata"))



} else {
  ## object called "dat_grid_final"
  load(file = here("data", "dat_fw_demo_lc_retail_750_pickuptype.Rdata"))
}
...
save(dat_grid_final, file = here("data","dat_fw_demo_lc_retail_shelters_750_pickuptype.Rdata"))



} else {
  ## object called "dat_grid_final"
  load(file = here("data", "dat_fw_demo_lc_retail_shelters_750_pickuptype.Rdata"))
}
...
if (gen_roads) {
  ...
  save(dat_grid_final, file = here("data","dat_fw_demo_lc_retail_shelters_roads_750_pickuptype.Rdata"))
  
} else {
  ## object called "dat_grid_final"
  load(file = here("data", "dat_fw_demo_lc_retail_shelters_roads_750_pickuptype.Rdata"))
}
...
dat_grid_final <- dat_grid_final %>%
  select(-name.y) ### remove "name" column. No use for it and is NA for all grid cells in pacific spirit ("lancaster_ub") transect. Causes problems.

dat_grid_final <- dat_grid_final %>%
  rename(site = name.x) ### remove "name" column. No use for it and is NA for all grid cells in pacific spirit ("lancaster_ub") transect. Causes problems.


dat_grid_final[] <- lapply(dat_grid_final, function(col) {
  if (is.matrix(col)) as.vector(col) else col
})


save(dat_grid_final, file = here("data","dat_fw_demo_lc_retail_shelters_roads_mods_750_pickuptype.Rdata"))


} else {
  ## object called "dat_grid_final"
  load(file = here("data", "dat_fw_demo_lc_retail_shelters_roads_mods_750_pickuptype.Rdata"))
}
...
# Identify numeric columns to scale (excluding ID columns)
cols_to_scale <- setdiff(names(data_only), c("site", "grid_id"))
numeric_cols_to_scale <- cols_to_scale[sapply(data_only[cols_to_scale], is.numeric)]
# Scale only numeric columns
data_only[numeric_cols_to_scale] <- lapply(data_only[numeric_cols_to_scale], scale)
...
dat_scaled$pickup_id <- as.integer(factor(dat_scaled$pickup_type))
...
dat_no_ub_scaled$pickup_id <- as.integer(factor(dat_no_ub_scaled$pickup_type))

dat_no_ub_scaled[] <- lapply(dat_no_ub_scaled, function(col) {
  if (is.matrix(col)) as.vector(col) else col
})


save(dat_no_ub_scaled, file = here("data", "dat_no_ub_scaled_750_pickuptype.Rdata"))
save(dat_no_ub, file = here("data", "dat_no_ub_750_pickuptype.Rdata"))


} else {
  ## object called "dat_scaled"
  load(file = here("data", "dat_no_ub_scaled_750_pickuptype.Rdata"))
}
...
# Save plot to PDF
pdf(here("figs","FA12_pickuptype.pdf"), width = 10, height = 8)
...
# Save plot to PDF
pdf(here("figs","FA13_pickuptype.pdf"), width = 10, height = 8)
...
# Save plot to PDF
pdf(here("figs","FA23_pickuptype.pdf"), width = 10, height = 8)
...
names(dat_final) <- gsub("\\[,*1\\]", "", names(dat_final))


save(dat_final, file = here("data", "dat_final_750_pickuptype.Rdata"))
save(loadings_matrix, file = here("data", "loadings_matrix_pickuptype.Rdata"))
