...
stan_data <- list(
  N = nrow(dat_combined),
  J = 12,
  site_id = dat_combined$site_id,
  X1 = dat_combined$Factor1,
  X2 = dat_combined$Factor2,
  X3 = dat_combined$Factor3,
  P1 = dat_combined$service_road_weight,
  fw_score_weighted = dat_no_ub$fw_score_weighted
)
```

```{r}
fit_hier_A_FA1_FA2_FA3_B_roadweight_noub <- stan(
...
```{r}
# Extract samples
posterior_samples <- rstan::extract(fit_hier_A_FA1_FA2_FA3_B_roadweight_noub)

# Extract posterior samples
y_rep <- posterior_samples$fw_score_pred  # Change "y_rep" based on how you named your predicted values
pi_pred <- posterior_samples$pi_pred # Change "y_rep" based on how you named your predicted values
y_obs <- dat_combined$fw_score_weighted  # Replace with actual observed data
pi_obs <- ifelse(dat_combined$fw_score_weighted > 0 , 0, 1)  # Convert to binary presence/absence
...
```{r}
fit_hier_A_FA1_FA2_B_roadweight_noub <- stan(
...
```{r}
save(fit_hier_A_FA1_FA3_B_roadweight_noub, file = here("data","fit_hier_A_FA1_FA3_B_roadweight_noub_corefactors.Rdata"))
```
```{r}
# Extract samples
